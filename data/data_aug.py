"""Data augment classes and functions
"""
import math
import random
...
class OneOrOther:
    ...
    def __call__(self, img, mask=None):
        if random.random() < self.prob:
            img, mask = self.first(img, mask)
        else:
            img, mask = self.second(img, mask)
        return img, mask
...
class RandomCrop:
    ...
    def __call__(self, img, mask=None):
        ...
        if mask is not None:
            ...
            mask = mask[h_start: h_start + self.h,
                        w_start: w_start + self.w, :]
...
class ShiftScale:
    ...
    def __call__(self, img, mask=None):
        ...
        if random.random() < self.prob:
            ...
            img = (img1[y1:y2, x1:x2, :]
                   if size == size0 else cv2.resize(img1[y1:y2, x1:x2, :], (size0, size0),
